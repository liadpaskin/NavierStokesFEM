      subroutine addmatvec ( p,ap,ve1,ve2,neq,ielblk,nsize,nd,nblock,numel,s1,s2,lm )
!c
!c.... routine to compute EBE matrix-vector multiplication
!c
      real*8    ap  ( 0 : neq ) ,p   ( 0 : neq ),ve1 ( nsize , nd  ),ve2 ( nsize , nd  ),&
            s1   (  nd  , nd , numel ),s2   (  nd  , nd , numel )
...
!c
        ...
        do j = 1 , nd
          do i = 1 , nd
            do ie = 1 , nvec
              ve2(ie,i) = ve2(ie,i) + (s1(i,j,iel0+ie)+s2(i,j,iel0+ie)) * ve1(ie,j)
            enddo
          enddo
        enddo
